...
def home():
    ...
    guessed_image_path = [{{ url_for('static', filename="images/cleaned_flags/" + str(x).lower() + ".png" ) }} for x in guessed_country_id]
    guessed_distances = [f"{int(round(x,0)/1000):,} km" if x != 0 else 0 for x in guessed_distances]
    guessed_directions_image_path = [{{ url_for('static', filename="images/cleaned_flags/" + str(x).lower() + ".png" ) }} for x in direction]
